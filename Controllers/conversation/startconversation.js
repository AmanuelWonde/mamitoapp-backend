            ...
            connection.query(sql, values, (error, result, fields) => {
                ...
                if (error) {
                    ...
                    if (result[0][0].status == 1021) {
                        reject(
                            new responseInstance(
                                new status(1021, documentation[1021]),
                                "the coversation with the user is already existed"
                            )
                        );
                    } else {
                        resolve(result[0][0]);
    ...
    getFCMtoken(result["user-2"]).then((fcmToken) => {
        ...
        fcm
            ...
            .send({
                token: fcmToken,
                data: {
                    custom_notification: JSON.stringify({ title: "new request", body: result["user-1"] })
                },
                android: {
                    priority: "high",
                },
