...
router.post("/signup", (req, res) => {
    ...
    return new Promise(async (resolve, reject) => {
      db.getConnection((error, connection) => {
        if (error) {
          reject(
            new responseInstance(
              new status(7001, "unable to connect to database"),
        ...
        connection.query(
          `CALL GetUserByUsername(\'${body.username}\')`,
          (error, result, fields) => {
            if (error) {
              debug("error", error);
              if (error)
            ...
            if (result[0].length > 0)
              reject(
                new responseInstance(
                  new status(6003, "username is unavailable"),
                  `the username ${body.username} is unavilable, try another`
                )
              );
            resolve(body);
          }
  ...
  function phoneNumberValidation(body) {
    ...
    return new Promise(async (resolve, reject) => {
      db.getConnection((error, connection) => {
        ...
        connection.query(
          `CALL GetUserByPhone(\'${body.phone}\')`,
          (error, result, fields) => {
            if (error) {
              debug("error", error);
              reject(new responseInstance(`Error: unknown issue`, error));
            ...
            if (result[0].length > 0)
              reject(
                new responseInstance(
                  new status(6004, "phone number is registered"),
                  `the phone number ${body.phone} is already registered, change phone number`
                )
              );
            resolve(body);
          }
    ...
    return new Promise(async (resolve, reject) => {
      bcrypt.genSalt(10, (err, salt) => {
        if (err) {
          ...
          reject(
            new responseInstance(
              new status(6010, "unable to generate salt"),
              "this is a backend issue"
            )
          );
        }
        bcrypt.hash(body.password, salt, (err, result) => {
          if (err) {
            ...
            reject(
              new responseInstance(
                new status(6011, "unable to hash password"),
                "this is a backend issue"
              )
            );
          }
    ...
    return new Promise((resolve, reject) => {
      db.getConnection((error, connection) => {
        if (error) {
          ...
          reject(
            new responseInstance(
              new status(6012, "unable to get pool connection"),
            ...
            if (error) {
              ...
              reject(
                new responseInstance(
                  new status(
                    6013,
                    "unable to save user's data to the database"
                  ),
                  "this is backend issue"
                )
              );
  ...
  function sender(body) {
    ...
    res
      ...
      .send(
        new responseInstance(
          new status(1000, "user registed"),
          "user is created successfully"
        )
      );
  }

  p1.then((body) => usernameValidation(body))
...
router.get("/login", (req, res) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      db.getConnection((error, connection) => {
        if (error) {
          ...
          connection.query(
            `${
              body.username_phone.match(/^251[7,9]\d{8}$/)
                ? `CALL GetUserByPhone(\'${body.username_phone}\')`
                : `CALL GetUserByUsername(\'${body.username_phone}\')`
            }`,
            (error, result, field) => {
              if (error) {
                debug(error);
                reject(
                  new status(6004, "user name not found"),
                  "retry with other username or create a new account"
                );
              } else {
                // debug(result[0])
  ...
  const userValidation = ({ result, body }) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (result.length > 0) {
        ...
        reject(
          new responseInstance(
            new status(6005, "invalid username"),
            "try a valid username and password"
          )
        );
      }
    });
    ...
    return new Promise((resolve, reject) => {
      bcrypt.compare(body.password, result.password, (error, res) => {
        if (error) {
          ...
          reject(
            new responseInstance(
              new status(6006, "invalid password"),
  ...
  const sender = (body) => {
    ...
    res
      ...
      .send(
        new responseInstance(
          new status(1001, "authentication successful"),
...
const auth = (req, res, next) => {
  ...
  let p1 = new Promise((resolve, reject) => {
    ...
    if (token) {
      ...
      reject(
        new responseInstance(
          new status(5001, "token is required"),
          "use a token inorder to get services"
        )
      );
    }
  });
  ...
  const tokenAuthentication = (token) => {
    try {
      ...
      reject(
        new responseInstance(
          new status(5002, "verification failed"),
          "use a valid toke in order to get services"
        )
      );
    }
  };
  ...
  p1.then((token) => tokenAuthentication(token))
